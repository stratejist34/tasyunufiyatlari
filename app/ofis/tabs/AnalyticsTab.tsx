...
export function AnalyticsTab() {
    ...
    return (
        <div className="space-y-8">
            <div>
                <p className="text-[11px] font-semibold uppercase tracking-[0.24em] text-cyan-300/80">Talep Analizi</p>
                            ...
                            <div className="space-y-0.5 mt-1">
                                {metrics!.eps_brands_7d.map((item, i) => (
                                    <div key={item.brand} className="grid grid-cols-[1.5rem_1fr_2.5rem_4rem_4rem] gap-x-2 px-1 py-1.5 rounded hover:bg-slate-800/40 transition-colors">
                                        ...
                                        <span className="text-[11px] text-sky-400/80 text-right">{formatM2(item.area_m2 ?? 0)}</span>
            ...
            <div className="grid gap-6 xl:grid-cols-2">
                ...
                <div className="admin-nexus-panel p-6">
                    <div className="flex items-center gap-2 mb-4">
                        <span className="flex h-5 w-5 items-center justify-center rounded text-[10px] font-bold bg-cyan-500/10 text-cyan-300 border border-cyan-500/20">E</span>
                        ...
                        {epsCombos.length > 0 ? epsCombos.map((item, index) => (
                            <div key={`${item.plate_brand}-${item.model}-${item.powder_brand}`} className="admin-nexus-subtle px-3 py-2.5">
                                <div className="flex items-center justify-between gap-3">
                                    <div className="flex items-center gap-2.5 min-w-0">
                                        ...
                                        <p className="text-xs text-slate-200 truncate">
                                            ...
                                            <span className="text-cyan-300/80">{item.powder_brand}</span>
                                        </p>
                                    </div>
                                ...
                                <div className="mt-1 ml-6 flex gap-2 text-[11px]">
                                    <span className="text-sky-400/80">{formatM2(item.area_m2 ?? 0)}</span>
