...
export function TrustStrip() {
  return (
    <section
      ...
      className="border-y border-fe-border/40 bg-fe-bg-2/40"
    >
          ...
          {ITEMS.map(({ Icon, title, body }) => (
            <li key={title} className="flex items-start gap-3">
              <Icon
                size={22}
                weight={ICON_WEIGHT}
                className="mt-0.5 shrink-0 text-fe-accent"
                aria-hidden
              />
              <div>
                <p className="text-sm font-semibold text-fe-text">{title}</p>
                <p className="mt-0.5 text-xs text-fe-text-muted leading-relaxed">{body}</p>
              </div>
            </li>
          ))}
        </ul>
