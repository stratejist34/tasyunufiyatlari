...
export function matchEpsPlateRow(
    ...
    const epsMaterialTypeId = materialTypes.find(m => m.slug === 'eps')?.id;
    ...
    const candidates = plates
        .filter(plate => epsMaterialTypeId === undefined || plate.material_type_id === epsMaterialTypeId)
        .map(plate => {
